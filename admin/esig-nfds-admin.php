...
if (!class_exists('ESIG_NFDS_Admin')) :
    ...
    class ESIG_NFDS_Admin extends ESIG_NF_SETTING {
        ...
        public function __construct() {
            ...
            add_action('ninja_forms_display_before_form', array($this, 'esig_ninja_forms_display_response'), 10);
        }
        ...
        final function esig_ninja_forms_display_response($form_id) {
        ...
        final function render_shortcode_esigninja($atts) {
            ...
            if (function_exists('esigget')) {
                ...
                $csum = (isset($_GET['csum'])) ? $_GET['csum'] : false;
            }
            ...
            //echo $field_id;
            $notification_id = WP_E_Sig()->meta->get($document_id, 'esig_ninja_notification_id');
            if (!$notification_id) {
                return;
            }

            $nf_value = self::get_value($document_id, $field_id);

            if (!$nf_value) {
                return;
            }
            $form_id = WP_E_Sig()->meta->get($document_id, 'esig_ninja_form_id');
            return self::display_value($notification_id, $form_id, $nf_value);
        }
